...
echo "Creating new index on instance '${INPUT_AZURESEARCHINSTANCE}':"
echo $DEFINITION | jq

set +e
RESPONSE=$(curl --fail-with-body -sSD/dev/stderr \
  --location \
  --request POST "https://${INPUT_AZURESEARCHINSTANCE}.search.windows.net/indexes?api-version=${API_VERSION}" \
